...
void FEUPConsulting::addAvailability(Student* student, string expertizeAvailable, unsigned cost) {
	Expertize not_found("",0);
	Expertize finding(expertizeAvailable,cost);
	Expertize found = this->expertizes.find(finding);

	if (found == not_found) {
		finding.addConsultant(student);
		this->expertizes.insert(finding);
	}
	else {
		this->expertizes.remove(found);
		found.addConsultant(student);
		this->expertizes.insert(found);
		return;
	}
...
vector<Student*> FEUPConsulting::getCandidateStudents(Project* project) const {
	...
	unsigned cost = project->getCost();
	string expert = project->getExpertize();

	BSTItrIn<Expertize> it(this->expertizes);

	while(!it.isAtEnd()) {
		Expertize elem = it.retrieve();

		if (elem.getName() == expert && elem.getCost() == cost) {
			vector<Student*> st = elem.getConsultants();

			for (size_t i = 0; i < st.size(); i++) {
				if (st[i]->getCurrentProject() == "") {
					temp.push_back(st[i]);
				}
			}
		}
